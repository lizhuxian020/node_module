...
class Bundle extends BundleBase {
  ...
  getSource(options: GetSourceOptions) {
    ...
    if (options.inlineSourceMap) {
      ...
      // 打包sourcemap路径可能会变，导致每次打包结果md5路径不一致
      if(options.dev){
  ...
  getSplitBundle() {
    if (!this.splitBundleInfo) {
      throw new Error('must define splitBundleInfo first');
    }
    this.assertFinalized();
    if (!this._splitBundle) {
      const {
        getBundleName,
        getModuleId,
        bundleNameList,
        ...
        nomalizeInPath,
        nomalizeOutPath,
      } = this.splitBundleInfo;
      const modules = this.getModules();
      const polyfillModules = [];
      const setupModules = [];
      const lazyModulesByBundle = bundleNameList.reduce((s, c) => (s[c] = [], s), {});
      modules.forEach(mod => {
        let {meta, sourcePath, name} = mod;
        if (meta && meta.preloaded) {
          // polyfill has no name
          (name ? setupModules : polyfillModules).push(mod);
        } else {
          const _mod = {path: sourcePath};
          lazyModulesByBundle[getBundleName(_mod)][getModuleId(_mod)] = mod;
        }
      });
      const dependencyInfo = Object.create(null);
      const tmpMod = {path: ''};
      const wrapBundle = (bundleName, bundleModules) => {
        const pathInfo = pathsInBundle[bundleName];
        const curDep = dependencyInfo[bundleName] = new Set();
        pathInfo.o.forEach(outPath => {
          tmpMod.path = outPath;
          curDep.add(getBundleName(tmpMod));
        })
        const codeStart = '!(function() {var __d=makeLocalDefine(' +
            JSON.stringify(bundleName) + ',' +
            JSON.stringify(pathInfo.i.map(nomalizeInPath)) + ',' +
            JSON.stringify(pathInfo.o.map(nomalizeOutPath)) + ',' +
            pathInfo.s + ');var require=__d.require;';
        const sourcePathStart = 'sub-bundle-begin-' + bundleName;
        const codeEnd = '})();';
        const sourcePathEnd = 'sub-bundle-end-' + bundleName;

        bundleModules.unshift(new ModuleTransport({
          id: 0,
          name: sourcePathStart,
          sourcePath: sourcePathStart,
          code: codeStart,
          sourceCode: codeStart,
          virtual: true,
        }));
        bundleModules.push(new ModuleTransport({
          id: 0,
          name: sourcePathEnd,
          sourcePath: sourcePathEnd,
          code: codeEnd,
          sourceCode: codeEnd,
          virtual: true,
        }));
      ...
      wrapBundle('preloaded', setupModules);
      const stringifyRegExp = obj =>
          '{'
        + Object.keys(obj)
            .map(k => JSON.stringify(k) + ':' + obj[k].toString())
            .join(',')
        + '}';
      const splitConfigModuleName = 'splitConfigModule';
      const splitConfigModuleCode = `;defineSubBundle(${stringifyRegExp(splitConfig)});`;
      const startupModules = polyfillModules.concat(new ModuleTransport({
        id: 0,
        name: splitConfigModuleName,
        sourcePath: splitConfigModuleName,
        code: splitConfigModuleCode,
        sourceCode: splitConfigModuleCode,
        virtual: true,
      }), setupModules);

      Object.keys(lazyModulesByBundle).forEach(bundleName => {
        if (lazyModulesByBundle[bundleName].length === 0) {
          delete lazyModulesByBundle[bundleName];
        } else {
          wrapBundle(bundleName, lazyModulesByBundle[bundleName]);
        }
      });
      this._splitBundle = {
        startupModules,
