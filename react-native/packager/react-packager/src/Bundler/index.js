...
class Bundler {
  ...
  _bundle({
    ...
    let modifyGetModuleId = () => true;
    if (splitConfig) {
      const sConfig = require(path.resolve(splitConfig));
      modifyGetModuleId = (bundle, transformedModules, response, modulesByName, runBeforeMainModule) => {
        const splitBundleInfo
            = bundle.splitBundleInfo
            = createSplitBundleInfo(
                this._projectRoots,
                sConfig,
                transformedModules,
                response,
                Array.isArray(runBeforeMainModule)
                  ? runBeforeMainModule
        ...
        response.getModuleId = splitBundleInfo.getModuleId;
        return true;
      }
    }
    const finalizeBundle = ({bundle, transformedModules, response, modulesByName}) =>
      ...
      Promise.all(
        ...
        let getRunBeforeMainModuleId
            = splitConfig
            ? mod => response.getModuleId(mod, {path: bundle.splitBundleInfo.entryFile})
            : response.getModuleId;
        const runBeforeMainModuleIds = Array.isArray(runBeforeMainModule)
          ? runBeforeMainModule
              .map(name => modulesByName[name])
              .filter(Boolean)
              .map(getRunBeforeMainModuleId)
          : undefined;

        // console.log(modulesByName[runBeforeMainModule[0]])
        // console.log(runBeforeMainModule)
        // console.log(runBeforeMainModuleIds)
        bundle.finalize({
          runMainModule,
...
function createSplitBundleInfo([projectRoot], splitConfig, transformedModules, response, runBeforeMainModulePaths) {
  const fileToBundleMap = Object.create(null);

  const PRELOADED = 'preloaded';
  const refs = new Set();
  const lazyBundleFiles = Object.create(null);
  const bundleNameList = [];
  const bundleRegList = [];
  for (let bundleName in splitConfig) {
    lazyBundleFiles[bundleName] = {
      i: new Set(),
      o: new Set(),
    };
    bundleNameList.push(bundleName);
    bundleRegList.push(splitConfig[bundleName]);
  }

  // init fileBundleMap
  const fileBundleMap = Object.create(null);
  transformedModules.forEach(({
    transformed: {
      sourcePath,
      meta: {
        preloaded
    ...
    if (preloaded) {
      fileToBundleMap[sourcePath] = PRELOADED;
    } else {
      for (let i = 0; i < bundleRegList.length; ++i) {
        if (bundleRegList[i].test(sourcePath)) {
          fileToBundleMap[sourcePath] = bundleNameList[i];
          return;
        }
      }
      console.log('分包配置有误，文件' + path + '不属于任何一个bundle')
      process.exit(1);
    }
  });

  function getBundleName({path}) {
    return fileToBundleMap[path];
  }

  // init preloaded and lazy modules
  let entryFile;
  let preloadedOutFilesSet = new Set();
  transformedModules.forEach(({
    module,
    transformed: {
      sourcePath,
    ...
    if (preloaded) {
      if (name) {
        // entry file
        if (entryFile) {
          console.log('entryFile has been defined with ' + entryFile);
          console.log('but ' + sourcePath + ' declear again');
          process.exit(1);
        }
        entryFile = sourcePath;
        runBeforeMainModulePaths.forEach(path => {
          preloadedOutFilesSet.add(path);
          refs.add(path);
        });
        if (dependencyPairs) {
          dependencyPairs.forEach(([, dep]) => {
            if (getBundleName(dep) !== PRELOADED) {
              preloadedOutFilesSet.add(dep.path);
              refs.add(dep.path);
            }
          });
        }
      ...
      let bundleName = getBundleName(module);
      const bundleSet = lazyBundleFiles[bundleName];
      bundleSet.i.add(sourcePath);
      if (dependencyPairs) {
        dependencyPairs.forEach(([, dep]) => {
          if (getBundleName(dep) !== bundleName) {
            bundleSet.o.add(dep.path);
            refs.add(dep.path);
          }
        });
      } else if (dependencies && dependencies.length) {
        response.getResolvedDependencyPairs(module).forEach(([, dep]) => {
          if (getBundleName(dep) !== bundleName) {
            bundleSet.o.add(dep.path);
            refs.add(dep.path);
          }
  ...
  const rootLength = projectRoot.length;
  const nomalizeCache = Object.create(null);
  const nomalizeOutPath = full => {
    let shortPath = nomalizeCache[full];
    if (!shortPath) {
      shortPath = nomalizeCache[full] = full.substring(rootLength);
    }
    return shortPath;
  }
  const nomalizeInPath = full => refs.has(full) ? nomalizeOutPath(full) : '';

  // sort and init fileToIdMap
  const pathsInBundle = Object.create(null);
  const fileToIdMap = Object.create(null);

  // preload
  const preloadIdMap = fileToIdMap[PRELOADED] = Object.create(null);
  preloadIdMap[entryFile] = 0;
  const preloadedOutFiles = Array.from(preloadedOutFilesSet).sort()
  preloadedOutFiles.forEach((path, id) => {
    preloadIdMap[path] = 1 + id;
  });
  pathsInBundle[PRELOADED] = {
    i: [entryFile],
    o: preloadedOutFiles,
    s: 1,
  }
  // lazy
  for (let bundleName in lazyBundleFiles) {
    const {
      i: inFiles,
      o: outFiles,
    } = lazyBundleFiles[bundleName];

    const idMap = fileToIdMap[bundleName] = Object.create(null);
    const pathList = pathsInBundle[bundleName] = {};
    const inList = Array.from(inFiles).sort();
    inList.forEach((path, id) => {
      idMap[path] = id;
    });
    pathList.i = inList;

    const outIdMapStartId = Math.ceil(inFiles.size * 2 / 100) * 100;
    pathList.s = outIdMapStartId;

    const outList = Array.from(outFiles).sort();
    outList.forEach((path, id) => {
      idMap[path] = outIdMapStartId + id;
    });
    pathList.o = outList;
  }

  const getModuleId = (mod, fromMod) => {
    return fileToIdMap[getBundleName(fromMod || mod)][mod.path];
  };
  return {
    getBundleName,
    getModuleId,
