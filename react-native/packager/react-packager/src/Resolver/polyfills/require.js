...
// 分包
global.defineSubBundle = defineSubBundle;
global.makeLocalDefine = makeLocalDefine;

const globalPathToModule: {[key: string]: (ModuleDefinition)} = Object.create(null);
const bundleNames: [string] = [];
const bundleRegExps: [RegExp] = [];

function defineSubBundle(splitConfig) {
  for (const name in splitConfig) {
    bundleNames.push(name);
    bundleRegExps.push(splitConfig[name]);
  }
}
function getBundleName(path) {
  for (let i = 0; i < bundleRegExps.length; ++i) {
    if (bundleRegExps[i].test(path)) {
      return bundleNames[i];
    }
  }
  throw new Error('Cannot find bundle for path: ' + path);
}
function makeLocalDefine(bundleName, innerIdToPath, outerIdToPath, outerStartId) {
  const modules: ModuleMap = Object.create(null);

  const loadModuleImplementation = function (moduleId, module) {
    const nativeRequireBundle = global.nativeRequireBundle;
    let outerPath;
    if (!module && nativeRequireBundle && moduleId >= outerStartId) {
      outerPath = outerIdToPath[moduleId - outerStartId];
      nativeRequireBundle(getBundleName(outerPath));
      module = globalPathToModule[outerPath];
    }

    if (!module) {
      throw unknownModuleError((outerPath || moduleId) + ' in ' + bundleName);
    }

    if (module.hasError) {
      throw moduleThrewError((outerPath || moduleId) + ' in ' + bundleName);
    }

    module.isInitialized = true;
    const exports = module.exports = {};
    const {factory, dependencyMap, localRequire} = module;
    try {

      const moduleObject: Module = {exports};

      factory(global, localRequire, moduleObject, exports, dependencyMap);
  ...
  let inGuard = false;
  const guardedLoadModule = function (moduleId: ModuleID , module) {
    if (!inGuard && global.ErrorUtils) {
      inGuard = true;
      let returnValue;
      try {
  ...
  const localRequire = function (moduleId: ModuleID) {
    let module = modules[moduleId];
    if (!module && moduleId >= outerStartId) {
      const outerPath = outerIdToPath[moduleId - outerStartId];
      module = modules[moduleId] = globalPathToModule[outerPath];
    }
    return module && module.isInitialized
      ? module.exports
  ...
  const localDefine = function (
    ...
    if (innerIdToPath[moduleId]) {
      globalPathToModule[innerIdToPath[moduleId]] = modules[moduleId];
    }
  }
  localDefine.require = localRequire;
  return localDefine;
}
