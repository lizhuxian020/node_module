...
@property (nonatomic, strong) RCTCustomRefreshControl *paffRefreshControl;
#endif
...
- (void)dockClosestSectionHeader
{
  ...
  if (_rctRefreshControl != nil && _rctRefreshControl.refreshing) {
    ...
    if (_paffRefreshControl != nil && _paffRefreshControl.refreshing) {
        scrollTop -= _paffRefreshControl.frame.size.height;
    }
...
- (void)setPaffRefreshControl:(RCTCustomRefreshControl *)paffRefreshControl{
    if (_paffRefreshControl) {
        [_paffRefreshControl removeFromSuperview];
    }
    _paffRefreshControl = paffRefreshControl;
    [self addSubview:_paffRefreshControl];
}
...
- (void)insertReactSubview:(UIView *)view atIndex:(NSInteger)atIndex
{
  ...
  else if ([view isKindOfClass:[RCTCustomRefreshControl class]]) {
      [_scrollView setPaffRefreshControl:(RCTCustomRefreshControl *)view];
  }
  else
...
- (void)removeReactSubview:(UIView *)subview
{
  ...
  } else if ([subview isKindOfClass:[RCTCustomRefreshControl class]]) {
      [_scrollView setPaffRefreshControl:nil];
  }
  else
#endif
...
- (void)layoutSubviews
{
  ...
  if (refreshControl && refreshControl.refreshing) {
    ...
    RCTCustomRefreshControl *paffRefreshControl = _scrollView.paffRefreshControl;
    if (paffRefreshControl && paffRefreshControl.refreshing) {
        paffRefreshControl.frame = (CGRect){_scrollView.contentOffset, {_scrollView.frame.size.width, paffRefreshControl.frame.size.height}};
    }
