                      ...
                      onComplete:(RCTJavaScriptCompleteBlock)onComplete
{
  ...
  [self registernativeRequireBundle];
...
- (void)registernativeRequireBundle
{
    [_performanceLogger addValue:0 forTag:RCTPLRAMNativeRequiresCount];
    [_performanceLogger addValue:0 forTag:RCTPLRAMStartupNativeRequires];
    [_performanceLogger addValue:0 forTag:RCTPLRAMStartupCodeSize];
    
    __weak RCTJSCExecutor *weakExecutor = self;
    _context.context[@"nativeRequireBundle"] = ^(NSString *bundleID) {
        RCTJSCExecutor *executor = weakExecutor;
        if (!executor || nil == bundleID || 0 == [bundleID length]) {
            return;
        }
        ...
        RCT_PROFILE_BEGIN_EVENT(RCTProfileTagAlways, ([@"nativeRequire_" stringByAppendingFormat:@"%@", bundleID]), nil);
        
        RCTJSCWrapper *jscWrapper = executor->_jscWrapper;
        NSString *nsCode;
        if(weakExecutor.delegate){
            nsCode = [weakExecutor.delegate loadBundleContentWithBundleId:bundleID];
        }else{
            nsCode = [NSString stringWithContentsOfFile:[[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"bundles/%@",bundleID] ofType:@"js"] encoding:NSUTF8StringEncoding error:nil];
        }
        [_performanceLogger addValue:[nsCode length] forTag:RCTPLRAMStartupCodeSize];
        JSStringRef code = jscWrapper->JSStringCreateWithUTF8CString([nsCode UTF8String]);
        JSValueRef jsError = NULL;
        JSStringRef sourceURL = jscWrapper->JSStringCreateWithUTF8CString([bundleID UTF8String]);
